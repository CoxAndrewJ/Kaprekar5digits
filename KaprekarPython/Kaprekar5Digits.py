...
def take_input():
    ...
    while True:
        # Take user input
        user_input = input("Please enter exactly 5 integers: ")
        ...
        if(len(user_input)==5):
            ...
            if(isinstance(user_input,int)):
                return user_input
        if(len(user_input)!=5):
            print("Please input exactly 5 integers")
...
## TODO -- Convert Constant check to a list, check to see if result already exists in list.
## If not, throw the result into the list, and continue
